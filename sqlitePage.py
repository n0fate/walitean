...
class SQLITE_PAGE():
	...
	def getcelloffset(self):
		...
		for count in range(0, self.header[2]):
			lstCelloffset.append(struct.unpack('>H', self.buf[REC_HEADER_SIZE+(count*2):REC_HEADER_SIZE+(count*2)+SIZEOFSHORT])[0])
	...
	def getData(self, celldata, type, size, offset):
		...
		if type == 'int':
			if size >= 6:
				...
				return struct.unpack('=b', data)[0] # signed int (6 or 8 bytes)
			elif size == 2:
				return struct.unpack('=h', data)[0] # signed int (6 or 8 bytes)
			elif size == 3 or size == 4:
				return struct.unpack('=i', data)[0] # signed int (6 or 8 bytes)
